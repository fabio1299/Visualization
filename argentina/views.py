...
def index(request):
    ...
    if filter is None:
        ...
        graph_list.append(pp.ModPerformance(GaugeDischarge01Min.objects.filter(code__in=filter).to_timeseries(index='mydate')))
...
def catchment_view(request,b=None):
    if b is None:
        ...
        catchment_as_geojson =catchment_as_geojson.replace('\\"type\\":\\"Polygon\\",\\"coordinates\\"','"type":"Polygon","coordinates"')
